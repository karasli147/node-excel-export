...
let buildExport = params => {
    ...
    params.forEach(function (sheet, index) {
        ...
        if (Array.isArray(specification)) {
            specification = specification.reduce((map, item) => {
                map.set(Object.keys(item)[0], Object.values(item)[0]);
                return map;
            }, new Map())
        } else {
            specification = Object.keys(specification).reduce((map, key) => {
                map.set(key, specification[key]);
                return map;
            }, new Map())
        }
